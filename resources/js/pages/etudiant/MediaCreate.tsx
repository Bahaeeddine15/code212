    ...
    <AppShell variant="sidebar">
      ...
      <div className="flex w-full min-h-screen">
        ...
        <AppContent variant="sidebar" className="overflow-x-hidden overflow-y-auto h-screen bg-white">
          <AppSidebarHeader breadcrumbs={[{ title: 'Dashboard Étudiant', href: '/dashboard' }, { title: 'Médiathèque', href: '/media' }, { title: 'Ajouter un média', href: '/media/create' }]} />
          <div className="container mx-auto px-4 py-6">
            <div className="flex items-center gap-4 mb-6">
              <Button variant="outline" onClick={() => router.visit('/media')}>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="mt-1">
                        ...
                        />
                        <p className="text-sm text-gray-500 mt-1">Formats acceptés: JPG, PNG, GIF, SVG, MP4, MOV, AVI (max 10MB)</p>
                      </div>
                      {errors.file && (
                    ...
                    <div className="flex gap-4">
                      <Button type="button" variant="outline" onClick={() => router.visit('/media')} className="flex-1">
                        Annuler
                      </Button>
                ...
                <CardContent>
                  {previewUrl ? (
                    <div className="space-y-4">
                      {data.file && (data.file as File).type.startsWith('video/') ? (
                        <video src={previewUrl} className="w-full max-h-64 rounded-lg border" controls />
                      ) : (
                        <img src={previewUrl} alt="Prévisualisation" className="w-full max-h-64 object-contain rounded-lg border" />
                      )}
                      <div className="text-sm text-gray-600">
                        <p>
                          <strong>Nom:</strong> {data.file ? (data.file as File).name : ''}
                        </p>
                        <p>
                          <strong>Taille:</strong> {data.file ? Math.round((data.file as File).size / 1024) : 0} KB
                        </p>
                        <p>
                          <strong>Type:</strong> {data.file ? (data.file as File).type : ''}
                        </p>
                      </div>
                    </div>
                  ) : data.file ? (
                    <div className="space-y-4">
                      <div className="w-full h-64 bg-gray-100 rounded-lg border flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <p className="text-gray-600">{(data.file as File).name}</p>
                        </div>
                      </div>
                      <div className="text-sm text-gray-600">
                        <p>
                          <strong>Nom:</strong> {(data.file as File).name}
                        </p>
                        <p>
                          <strong>Taille:</strong> {Math.round((data.file as File).size / 1024)} KB
                        </p>
                        <p>
                          <strong>Type:</strong> {(data.file as File).type}
                        </p>
                      </div>
                    </div>
                  ) : (
